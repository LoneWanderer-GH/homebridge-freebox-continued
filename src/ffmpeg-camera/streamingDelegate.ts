/**
 *
 * TAKEN FROM https://github.com/Sunoo/homebridge-camera-ffmpeg/blob/master/src/streamingDelegate.ts
 *
 *
 */
...
export class StreamingDelegate implements CameraStreamingDelegate {
  ...
  constructor(
    ...
    this.logger = new PluginLogger(this.log, 'StreamingDelegate');
  ...
  fetchSnapshot(snapFilter?: string): Promise<Buffer> {
    this.snapshotPromise = new Promise((resolve, reject) => {
      ...
      this.logger.debug('Snapshot command: ' + this.videoProcessor + ' ' + ffmpegArgs, this.cameraName, this.videoConfig.debug);
      ...
      ffmpeg.stderr.on('data', (data) => {
        data.toString().split('\n').forEach((line: string) => {
          if (this.videoConfig.debug && line.length > 0) { // For now only write anything out when debug is set
            this.logger.error(line, this.cameraName + '] [Snapshot');
          }
        });
      });
      ffmpeg.on('close', () => {
        ...
        if (runtime < 5) {
          this.logger.debug(message, this.cameraName, this.videoConfig.debug);
        } else {
          if (!this.unbridge) {
            message += ' It is highly recommended you switch to unbridge mode.';
          }
          if (runtime < 22) {
            this.logger.warn(message, this.cameraName);
          } else {
            message += ' The request has timed out and the snapshot has not been refreshed in HomeKit.';
            this.logger.error(message, this.cameraName);
          }
        }
      });
    });
  ...
  resizeSnapshot(snapshot: Buffer, resizeFilter?: string): Promise<Buffer> {
    return new Promise<Buffer>((resolve, reject) => {
      ...
      this.logger.debug('Resize command: ' + this.videoProcessor + ' ' + ffmpegArgs, this.cameraName, this.videoConfig.debug);
      const ffmpeg = spawn(this.videoProcessor, ffmpegArgs.split(/\s+/), { env: process.env });
  ...
  async handleSnapshotRequest(request: SnapshotRequest, callback: SnapshotRequestCallback): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Snapshot requested: ' + request.width + ' x ' + request.height,
        this.cameraName, this.videoConfig.debug);

      const snapshot = await (this.snapshotPromise || this.fetchSnapshot(resolution.snapFilter));

      this.logger.debug('Sending snapshot: ' + (resolution.width > 0 ? resolution.width : 'native') + ' x ' +
      ...
      this.logger.error(err as string, this.cameraName);
      callback();
    }
  }

  async prepareStream(request: PrepareStreamRequest, callback: PrepareStreamCallback): Promise<void> {
    this.logger.debug('Preparing stream with session ID ' + request.sessionID, this.cameraName);
    ...
    //   reserveTimeout: 15,
    // };
    const videoReturnPort: number = await pickPort({
      type: 'udp',
      ip: ipv6 ? '::' : '0.0.0.0',
      reserveTimeout: 15,
    });
    const videoSSRC = this.hap.CameraController.generateSynchronisationSource();
    const audioReturnPort: number = await pickPort({
      type: 'udp',
      ip: ipv6 ? '::' : '0.0.0.0',
      reserveTimeout: 15,
    });
  ...
  private startStream(request: StartStreamRequest, callback: StreamRequestCallback): void {
    this.logger.debug('Starting stream with session ID ' + request.sessionID, this.cameraName);
    ...
    if (sessionInfo) {
      ...
      this.logger.debug('Video stream requested: ' + request.video.width + ' x ' + request.video.height + ', ' +
                request.video.fps + ' fps, ' + request.video.max_bit_rate + ' kbps', this.cameraName, this.videoConfig.debug);
      this.logger.info('Starting video stream: ' + (resolution.width > 0 ? resolution.width : 'native') + ' x ' +
      ...
      ffmpegArgs += // Video
                ...
                ' -g ' + Math.max(1, Math.round(fps * 2)) +
                ' -keyint_min ' + Math.max(1, Math.round(fps)) +
      ...
      if (this.videoConfig.audio) {
        if (request.audio.codec === AudioStreamingCodecType.OPUS || request.audio.codec === AudioStreamingCodecType.AAC_ELD) {
          ...
          this.logger.error('Unsupported audio codec requested: ' + request.audio.codec, this.cameraName);
        }
      }
      ...
      activeSession.socket.on('error', (err: Error) => {
        this.logger.error('Socket error: ' + err.message, this.cameraName);
      ...
      activeSession.socket.on('message', () => {
        ...
        activeSession.timeout = setTimeout(() => {
          this.logger.info('Device appears to be inactive. Stopping stream.', this.cameraName);
      ...
      this.logger.error('Error finding session information.', this.cameraName);
  ...
  public stopStream(sessionId: string): void {
    this.logger.debug('Stopping stream with session ID ' + sessionId, this.cameraName);
    ...
    if (session) {
      ...
      try {
        ...
        this.logger.error('Error occurred closing socket: ' + err, this.cameraName);
      }
      try {
        session.mainProcess?.stop();
      } catch (err) {
        this.logger.error('Error occurred terminating main FFmpeg process: ' + err, this.cameraName);
      }
      try {
        session.returnProcess?.stop();
      } catch (err) {
        this.logger.error('Error occurred terminating two-way FFmpeg process: ' + err, this.cameraName);
      }
    }
    this.ongoingSessions.delete(sessionId);
    this.logger.info('Stopped video stream.', this.cameraName);
  }
