...
export class FfmpegProcess {
  ...
  constructor(
    ...
    this.logger = new PluginLogger(this.log, 'FfmpegProcess');

    this.log.debug('Stream command: ' + videoProcessor + ' ' + ffmpegArgs, cameraName, debug);
    ...
    this.process.stdout.on('data', (data) => {
      ...
      if (progress) {
        if (!started && progress.frame > 0) {
          ...
          if (runtime < 5) {
            this.log.debug(message, cameraName, debug);
          } else if (runtime < 22) {
    ...
    stderr.on('line', (line: string) => {
      ...
      if (debug && line.match(/\[(panic|fatal|error)\]/)) { // For now only write anything out when debug is set
        ...
        this.log.debug(line, cameraName, true);
      }
    });
    ...
    this.process.on('exit', (code: number, signal: NodeJS.Signals) => {
      ...
      if (this.killTimeout && code === 0) {
        this.log.debug(message + ' (Expected)', cameraName, debug);
      } else if (code === null || code === 255) {
        if (this.process.killed) {
          this.log.debug(message + ' (Forced)', cameraName, debug);
        } else {
          this.log.error(message + ' (Unexpected)', cameraName);
        }
      } else {
